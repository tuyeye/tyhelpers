...
import { Image, Input, notification, Space } from 'antd';
    ...
    <>
      <Input.TextArea
        ...
        onChange={(e) => {
          ...
          // 设置 FormItem
          if (onFormItemChange) {
            onFormItemChange(e.target.value);
          }
        }}
        onBlur={onBlur}
  ...
  return (
    <>
          ...
          <>
            {src && (
              <img
                src={src}
                onError={ImgError}
                className="tyhelpers-media-img"
              />
            )}
            {!src && (
              <div className="tyhelpers-media-empty-container">
      ...
      <Image
        src={src}
        style={{ display: 'none' }}
        preview={{ visible: preVisible, onVisibleChange: setPreVisible }}
      />
      {contextHolder}
    </>
  );
...
const MediaAudio: FC<FormItemProps & UsualProps> = (props) => {
  ...
  return (
    <>
      <div
        className="tyhelpers-media-container"
        style={{ background: 'rgb(241,243,244)', ...style, minHeight: 250 }}
      >
        {editMode && (
          <InputMode
            value={src}
