...
import {
    ProTable, ProTableProps, ProColumns, ActionType,
    ProFormProps, ProFormColumnsType, FormInstance
} from '@ant-design/pro-components';
import type { ParamsType } from '@ant-design/pro-provider';
import { Popconfirm } from 'antd';
import JsonProTableUpdate, { JsonProTableUpdateType } from './JsonProTableUpdate';
...
export interface JsonProTableProps<T, U, ValueType> extends ProTableProps<T, U, ValueType> {
...
>(props: JsonProTableProps<DataType, Params, ValueType>) => {
    ...
    const [tableProps, setTableProps] = useState<ProTableProps<DataType, Params, ValueType>>();
    ...
    function createOptionCell(_columns: ProColumns<DataType, ValueType>[]) {
        ...
        if (_props) {
            ...
            const closeOptionCell = (_columns1.length === 0) && (!_props.onRemove) && (!props.JsonProTableUpdateProps) && (!_customOptions);

            if (!closeOptionCell) {

                _columns0.push(
                    {
                        onHeaderCell: () => ({ style: { textAlign: "center", minWidth: 150 } }),
                        ...
                        render: (text, row, index, action, f) => {
                            ...
                            _columns1.map(e => {
                                if (e.render) {
                                    const theNode = e.render(text, row, index, action, f) as JSX.Element[];
                            ...
                            if (props.JsonProTableUpdateProps) {
                                _userOptionNodes.push(
                                    ...
                                    <a key="edit" type="link" onClick={() => {
                                        Promise.resolve(setUpdateRow(row)).then(() => {
                                            updateRef.current?.setUpdateVis(true)
                                        });
                                    }
                                    }>{props.JsonProTableProps?.customUpdateText ?? "编辑"}</a>
                                )
                            }

                            if (_props.onRemove) {
                                _userOptionNodes.push(<Popconfirm
                                    placement="left"
                                    key="delete"
                                    title={<>{_props.removePopconfirmTitle ?? "确定要删除吗？"}</>}
                                    onConfirm={async () => {
                                        if (_props.onRemove) {
    ...
    useEffect(() => {
        ...
        if (_tableProps && (!_tableProps.JsonProTableLoading)) {
            ...
            Promise.resolve(_tableProps.columnsRequest(_stringToProps).then(res => {
    ...
    if ((!(tableProps && columns)) || props.JsonProTableProps?.JsonProTableLoading) return props.JsonProTableProps?.customTableLoadingSpinNode ?? <BackDrop open tip="表格生成中，请稍候片刻..." />;
    return <>
        <ProTable<DataType, Params, ValueType>
            actionRef={tableRef}
            {...tableProps}
            columns={columns}
            toolBarRender={((!props.toolBarRender) && (!props.JsonProTableCreateProps)) ? (
                props.toolBarRender === false ? false : (() => [])
            ) : (
                (action, rows) => [
                    ...(props.toolBarRender ? props.toolBarRender(action, rows) : []),
                    ...(props.JsonProTableCreateProps ? [<JsonProTableCreate
                        key="create"
