...
import { BetaSchemaForm, ActionType, ProColumns, DragSortTable, DrawerForm, ProForm, ProCard, ProTable } from '@ant-design/pro-components';
...
export declare type JsonFormItemEditerProps = {
     ...
     */
    searchDataRequest: (params: any) => Promise<{ data: JsonFormItemDataSourceType[], total: number }>
     ...
     */
    changeIndexRequest: (newList: JsonFormItemDataSourceType[]) => Promise<boolean>;
     ...
     * @param row 所选行的数据
     */
    updateRequest: (data: JsonFormItemDataSourceType, row: JsonFormItemDataSourceType) => Promise<boolean>;
};

function JsonFormItemEditer(props: JsonFormItemEditerProps) {

    const { dataRequest, postRequest, deleteRequest, changeIndexRequest, copyItemRequest, searchDataRequest, updateRequest } = props;
    ...
    return (<ProCard title={`Item List`} extra={<a onClick={() => { setSearchVis(true) }}>从其他 form 中复制</a>}>
        <Modal
            visible={searchVis}
            onCancel={() => { setSearchVis(false) }}
        ...
        <DrawerForm
            ...
            title="编辑更多信息"
            visible={moreVisible}
            drawerProps={{ onClose: () => setMoreVisible(false), destroyOnClose: true }}
            width={500}
            onFinish={async (data) => {
                const res = await BackDrop.promise("处理中", async () => {
        ...
        >
            <BetaSchemaForm<JsonFormItemDataSourceType> columns={columns as any} layoutType="Embed" />
        </DrawerForm>
